...
var roleUTransfer = require('role.utransfer');
...
module.exports.loop = function () {
    ...
    if(!Game.spawns['Spawn1'].spawning) {
        if(energyAvailable >= 1250) {
            ...
            if(harvesters.length < 1) {
                ...
                Game.spawns['Spawn1'].spawnCreep([WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,MOVE,MOVE,MOVE,MOVE,MOVE], UHnewName, 
                    {memory: {role: 'uharvester'}});
            }

            else if(utransfers.length < 1) {
                var UTnewName = 'UTransfer' + Game.time;
                console.log('Spawning new utransfer: ' + UTnewName);
                Game.spawns['Spawn1'].spawnCreep([CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE], UTnewName,
                    {memory: {role: 'utransfer'}});
            }
    ...
    for(var name in Game.creeps) {
        ...
        if(creep.memory.role == 'utransfer') {
            roleUTransfer.run(creep);
        }
